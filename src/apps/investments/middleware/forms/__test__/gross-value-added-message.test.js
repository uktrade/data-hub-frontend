const {
  ...
  gvaMessages,
} = require('../gross-value-added-message')
const { investmentTypes } = require('../../../types')

describe('Gross value added message', () => {
  context(
    'when the investment type is not Foreign direct investment (FDI)',
    () => {
  ...
  context(
    'when GVA is null and the user has not provided Foreign equity investment',
    () => {
      it('should provide a message to the user', () => {
        ...
        expect(grossValueAddedMessage(investment)).to.eq(
          gvaMessages.capitalExpenditureRequired
        )
      })
    }
  )
  ...
  context('when GVA is null and the user has not provided a Sector', () => {
    it('should provide a message to the user', () => {
      ...
      expect(grossValueAddedMessage(investment)).to.eq(
        gvaMessages.primarySector
      )
    })
  })

  context(
    'when GVA is null and the user has not provided both Foreign equity investment and a Sector ',
    () => {
      it('should provide a message to the user', () => {
        const investment = {
          sector: null,
        ...
        expect(grossValueAddedMessage(investment)).to.eq(
          gvaMessages.capitalExpenditureAndPrimarySectorRequired
        )
      })
    }
  )
