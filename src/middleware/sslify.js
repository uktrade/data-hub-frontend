 ...
 * @api private
 */
function checkSecure (req, settings) {
  ...
  if (!isHttps && settings.trustProtoHeader) {
  ...
  if (!isHttps && settings.trustAzureHeader && req.headers['x-arr-ssl']) {
    isHttps = true
  }
 ...
 * @api public
 */
const setSecure = function setSecure (options) {
  return function (req, res, next) {
    const settings = Object.assign(defaults, options)

    // set secure status on request for use in other middlewares
    req.isHttps = checkSecure(req, settings)
 ...
 * @api public
 */
const enforceHTTPS = function enforceHTTPS (options) {
  return function (req, res, next) {
    const settings = Object.assign(defaults, options)
    const isHttps = checkSecure(req, settings)
    ...
    if (isHttps) {
      ...
      if (req.method === 'GET' || req.method === 'HEAD') {
        var host = settings.trustXForwardedHostHeader ? (req.headers['x-forwarded-host'] || req.headers.host) : req.headers.host
