import { keys, forEach, isObject } from 'lodash'
import { stringify } from 'qs'

import { keyValueTable, localHeader, tabbedLocalNav } from '../selectors'
...
const assertKeyValueTable = (dataTest, expected) => {
  forEach(keys(expected), (key, i) => {
    ...
    if (expected[key] === null) {
      cy.get(keyValueTable(dataTest).valueCell(rowNumber)).should(
        'have.text',
        key
      )
    } else if (expected[key].href) {
      cy.get(keyValueTable(dataTest).keyCell(rowNumber)).should(
        'have.text',
        key
      )
      cy.get(keyValueTable(dataTest).valueCellLink(rowNumber)).should(
        'have.attr',
        'href',
        expected[key].href
      )
...
const assertLocalHeader = (header) => {
  cy.get(localHeader).contains(header)
}

const assertTabbedLocalNav = (nav) => {
  cy.get(tabbedLocalNav).contains(nav)
}
