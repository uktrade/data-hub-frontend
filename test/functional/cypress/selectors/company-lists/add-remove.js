module.exports = {
  addRemoveButton: `.c-local-header__actions a`,
  listA: {
    legend: `#add-remove-list-form > form > div > div:nth-child(2) > div > fieldset > legend`,
    radios:
      '#add-remove-list-form > form > div > div:nth-child(2) > div > fieldset > div input',
    labelYes:
      '#add-remove-list-form > form > div > div:nth-child(2) > div > fieldset > div > div:nth-child(2) > label',
    labelNo:
      '#add-remove-list-form > form > div > div:nth-child(2) > div > fieldset > div > div:nth-child(3) > label',
  },
  listB: {
    legend: `#add-remove-list-form > form > div > div:nth-child(3) > div > fieldset > legend`,
    radios:
      '#add-remove-list-form > form > div > div:nth-child(3) > div > fieldset > div input',
    labelYes:
      '#add-remove-list-form > form > div > div:nth-child(3) > div > fieldset > div > div:nth-child(2) > label',
    labelNo:
      '#add-remove-list-form > form > div > div:nth-child(3) > div > fieldset > div > div:nth-child(3) > label',
  },
  listC: {
    legend: `#add-remove-list-form > form > div > div:nth-child(4) > div > fieldset > legend`,
    radios:
      '#add-remove-list-form > form > div > div:nth-child(4) > div > fieldset > div input',
    labelYes:
      '#add-remove-list-form > form > div > div:nth-child(4) > div > fieldset > div > div:nth-child(2) > label',
    labelNo:
      '#add-remove-list-form > form > div > div:nth-child(4) > div > fieldset > div > div:nth-child(3) > label',
  },
  createButton: '#add-remove-list-form > form > div > a',
  saveButton: '#add-remove-list-form > form div div:nth-child(6) button',
  cancelLink: '#add-remove-list-form > form div div:nth-child(6) a',
}
