...
describe('Interaction options', () => {
  beforeEach(() => {
    ...
    this.res = { locals: { 'interaction.created_on': '2018-01-23T10:39:18.220321Z' } }
  ...
  context('when there is no tap services', () => {
    beforeEach(() => {
      this.interactionOption = proxyquire('~/src/apps/interactions/services/interaction-options', {
        ...
        getCommonOptions: this.getCommonOptionsStub.resolves(this.commonOptions),
      })
    })

    it('should return an empty list for tapServices', async () => {
      const formOptions = await this.interactionOption.getInteractionOptions(this.req, this.res)
