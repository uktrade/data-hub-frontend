...
describe('Global builders', () => {
  ...
  describe('#buildNavObject', () => {
    ...
    it('should return and object with values from navItem when it matches permitted applications', () => {
      ...
      const actual = this.builders.buildNavObject(this.req, this.navItem, this.permittedApps)

      expect(actual.label).to.equal(matchedObject.label)
      expect(actual.url).to.equal(matchedObject.url)
      expect(actual.isActive).to.equal(matchedObject.isActive)
    })

    it('should return and object with values from navItem when permitted applications list is empty but bypassSSO is enabled', () => {
      const matchedObject = {
        label: 'Perseus',
        url: '/perseus',
        isActive: true,
      ...
      const bypassSSO = true

      const actual = this.builders.buildNavObject(this.req, this.navItem, permittedAps, bypassSSO)
