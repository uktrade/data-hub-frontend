...
class EditInternalDetailsController extends EditController {
  configure(req, res, next) {
    const filteredServiceTypes = filter(
      metadataRepo.orderServiceTypesOptions,
      (service) => {
        if (!service.disabled_on) {
          return true
        }
    ...
    req.form.options.fields.sector.options = metadataRepo.sectorOptions.map(
      transformObjectToOption
    )
    req.form.options.fields.service_types.options = filteredServiceTypes.map(
      transformObjectToOption
    )
  ...
  process(req, res, next) {
    req.form.values.service_types = filter(
      flatten([req.form.values.service_types])
    )
    next()
  }
}
