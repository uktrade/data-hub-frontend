...
When(/^I choose the (.+) country option$/, async (country) => {
  await Event.setValue('@addressCountry', '').clickListOption(
    'address_country',
    country
  )
})
...
When(/^I add it to the programmes list$/, async () => {
  await Event.click('@addAnotherProgramme')
})

Then(
  /^I verify there should be ([0-9]) programmes lists$/,
  async (expected) => {
    await Event.assertVisibleRelatedProgrammesList(expected)
  }
)

Then(/^I verify there is the option to add another programme/, async () => {
  await Event.assert.visible('@addAnotherProgramme')
})

Then(/^I verify the event save button is displayed$/, async () => {
  await Event.assert.visible('@saveButton')
})

Then(/^I verify the event name has an error message$/, async () => {
  await Event.assert.visible('@eventNameError')
})
...
Then(/^I verify the event UK region has an error message$/, async () => {
  await Event.assert.visible('@ukRegionError')
})

Then(
  /^I see the event is displayed correctly with all field values$/,
  async () => {
    await Event.assert
      .containsText('@eventNameFromDetails', '')
