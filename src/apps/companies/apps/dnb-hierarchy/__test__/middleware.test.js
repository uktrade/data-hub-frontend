...
describe('D&B Hierarchy middleware', () => {
  ...
  describe('#setDnbHierarchyDetails', () => {
    context(
      'when the company does have a DUNS number and the feature flag is on',
      () => {
        mockGetDnbHierarchy({
        ...
        it('should set "dnbHierarchyCount"', () => {
          expect(
            middlewareParameters.resMock.locals.dnbHierarchyCount
          ).to.equal(2)
        })
    ...
    context(
      'when the company does not have a DUNS number and the feature flag is on',
      () => {
        const middlewareParameters = buildSubsidiaryMiddlewareParameters({
          company: {
            duns_number: null,
    ...
    context('when the feature flag is off', () => {
      ...
      it('should not set "dnbHierarchyCount"', () => {
        expect(middlewareParameters.resMock.locals.dnbHierarchyCount).to.be
          .undefined
      })

      it('should set "dnbRelatedCompaniesCount" equal to 0', () => {
        expect(middlewareParameters.resMock.locals.dnbRelatedCompaniesCount).to
          .be.undefined
      })
    })
  })
