...
When(/^a service delivery policy feedback is added$/, async function(
  dataTable
) {
  const details = fromPairs(
    map(dataTable.hashes(), (hash) => [camelCase(hash.key), hash.value])
  )
  await Interaction.createInteractionPolicyFeedback(
    details,
    true,
    (interaction) => {
...
When(/^[an]{1,2} (interaction|service delivery) is added$/, async function(
  kind,
  dataTable
) {
  const details = fromPairs(
...
When(/^I select export interaction$/, async function() {
  await Interaction.waitForElementVisible('@continueButton')
    .click('@export')
...
When(/^I select a policy feedback option$/, async function() {
  await Interaction.waitForElementVisible('@policyFeedbackYes').click(
    '@policyFeedbackYes'
  )
})

When(/^the interaction events Yes option is chosen$/, async function() {
  await Interaction.setValue('@eventYes', '').click('@eventYes')
})

When(/^the interaction events No option is chosen$/, async function() {
  await Interaction.setValue('@eventNo', '').click('@eventNo')
})

Then(/^there are interaction fields$/, async function() {
  await Interaction.waitForElementVisible('@contact')
...
Then(/^there are interaction policy feedback fields$/, async function() {
  await Interaction.waitForElementVisible('@policyIssueType1')
...
Then(/^(interaction|policy feedback) fields are pre-populated$/, async function(
  kind
) {
  const assertIsSet = (result) =>
 ...
 */
Then(
  /^I filter the collections to view the (.+) I have just created$/,
  async function(typeOfInteraction) {
    ...
    const inputDateFormat = process.env.NW_CIRCLECI
      ? 'MM/DD/YYYY'
      : 'DD/MM/YYYY'
    ...
    await filterTagsSection.waitForElementPresent('@kind', waitForTimeout)

    client.clearValue('#field-date_after')

    await filtersSection.sendKeys('@dateFrom', date)

    client.expect
      .element('#field-date_after')
      .to.have.value.which.contains(`${expectedDate}`)

    client.clearValue('#field-date_before')

    await filtersSection.sendKeys('@dateTo', date)

    client.expect
      .element('#field-date_before')
      .to.have.value.which.contains(`${expectedDate}`)

    await filtersSection.sendKeys('@dateTo', [client.Keys.ENTER])

    await filterTagsSection.waitForElementPresent('@dateTo', waitForTimeout)
  }
)
