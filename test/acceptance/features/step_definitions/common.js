...
When(
  /^I (?:navigate|go|open|visit).*? `(.+)` page using `(.+)` `(.+)` fixture$/,
  async function(pageName, entity, fixtureName) {
    const page = get(client.page, pageName)()
    ...
    set(
      this.state,
      entityTypeKey,
      assign({}, get(this.state, entityTypeKey), fixture)
    )
...
When(/^I select a value for `(.+)` on the `(.+)` page$/, async function(
  elementName,
  pageName
) {
  try {
    const page = get(client.page, pageName)()
    await page.getListOption(`@${elementName}`, (item) => {
...
Then(/^I should not see the "([^"]*)?" (link|button)$/, async function(
  linkText,
  type
) {
  await client
...
Then(/^I should see the "([^"]*)?" (link|button)$/, async function(
  linkText,
  type
) {
  await client
...
Then(/^I should see the correct text on the `(.+)` page$/, async function(
  pageName,
  data
) {
  try {
