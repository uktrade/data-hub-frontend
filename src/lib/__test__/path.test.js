...
describe('#joinPaths', () => {
  ...
  context(
    'when the last path has an extra forward slash at the beginning',
    () => {
      beforeEach(() => {
        this.actual = joinPaths(['/first', 'second', '/third'])
      })
  ...
  context(
    'when the paths other than the first and last have extra forward slashes',
    () => {
      beforeEach(() => {
        this.actual = joinPaths([
          '/first',
          '/second/',
          '/third/',
          '/fourth/',
