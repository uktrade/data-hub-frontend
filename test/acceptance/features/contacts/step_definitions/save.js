...
When(/^a primary contact is added$/, async function() {
  await Contact.createNewContact({}, true, (contact) => {
...
When(/^a primary contact with new company address is added$/, async function() {
  await Contact.createNewPrimaryContactWithNewCompanyAddress({}, (contact) => {
    set(this.state, 'contact', contact)
  })
})

When(/^a non-primary contact is added$/, async function() {
  await Contact.createNewContact({}, false, (contact) => {
    set(this.state, 'contact', contact)
    set(this.state, 'contact.address', getAddress(this.state.company))
  })
})
...
When(/^the contact is clicked/, async function() {
  await Company.section.firstContactsTabContact.click('@header')
})

Then(/^there are contact fields$/, async function() {
  // TODO this can be DRY'd up to use a generic datatable to assert what the form contains
  await Contact.waitForElementVisible('@firstName')
...
Then(/^the contact is displayed on the company contact tab$/, async function() {
  ...
  await Company.section.firstContactsTabContact.assert
    .visible('@header')
...
Then(/^the contact is displayed on the dashboard$/, async function() {
  ...
  await Dashboard.assert.containsText('@firstMyLatestContact', contactName)
})
