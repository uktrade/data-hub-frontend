...
const ExportInteractionsDetails = () => {
  ...
  const exportInteractionId = matchId && matchId[1]

  return (
    <InteractionResource id={exportInteractionId}>
      {(exportInteraction) => (
        <DefaultLayout
          heading={exportInteraction.subject}
          pageTitle="Export interaction details"
          breadcrumbs={[
            { link: urls.exportPipeline.index(), text: 'Home' },
            {
              link: urls.exportPipeline.interactions.index(
                exportInteraction.company_export.id
              ),
              text: 'Interactions',
            },
            { text: `${exportInteraction.subject}` },
          ]}
        >
          {/*
        ...
        {(exportInteraction) => ()}
          ...
          <StyledSummaryTable>
            ...
            <SummaryTable.Row heading="Adviser(s)">
              <ul>
                {exportInteraction.dit_participants.map(
                  (participant, index) => (
          ...
          <ActionsContainer>
            ...
            <StyledLink
              data-test="back"
              href={urls.exportPipeline.interactions.index(
                exportInteraction.company_export.id
              )}
            >
              Back
